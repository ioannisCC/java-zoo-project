package com.ioannis.unipiZoo;

public class Giraffe extends Animal {
    private double height;
    private static int counter=0;  //static because it has to count the instances of this specific class in order to give different id to each instance

    public Giraffe(String name, String homotaxy, double weight, int age, double height) {
        super(name, homotaxy, weight, age);
        this.height = height;
        setClassname("giraffe");
        counter+=1;
        if (counter < 100) setId("gir"+00+counter);
        else if (counter >= 100) setId("gir"+00+counter);
    }

    @Override
    public double getHeight() {
        return height;
    }
    @Override
    public void setHeight(double height) {
        this.height = height;
    }

    @Override
    public void makeSound()
    {
        System.out.println("-joui joui joui joui-");

        try {
            Thread.sleep(10000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }

    @Override
    public void rarity()
    {
        if (counter<3)
            System.out.println("animal is rare in our zoo");
        else
            System.out.println("animal is common in our zoo");

        try {
            Thread.sleep(10000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }

    @Override
    public String toString() {
        return super.toString() +
                "height=" + height +
                '}';
    }
}
